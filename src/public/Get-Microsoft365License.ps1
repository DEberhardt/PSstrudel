...
#TODO Add ParameterNames to Licenses that were added recently and are relevant (IncludesTeams, IncludesPhoneSystem)
#TODO check Teams Phone Standard and others.

function Get-Microsoft365License {
  <#
  .SYNOPSIS
    License information for Microsoft 365 Licenses related to Teams
  .DESCRIPTION
  ...
  .EXAMPLE
    Get-Microsoft365License

    Returns Microsoft 365 Licenses that relate to Teams for use in other commands
  .INPUTS
    System.String
  .OUTPUTS
    System.Object
  ...
  .LINK
    https://github.com/DEberhardt/PSstrudel/tree/main/docs/cmdlet/Get-Microsoft365License.md
  .LINK
  ...
  [CmdletBinding()]
  [Alias('Get-AzureAdLicense', 'Get-License')]
  [OutputType([Object[]])]
  param(
    [Parameter()]
    [String]$SearchString,
  ...
  begin {
    ...
    $ProductsNotAdded = [System.Collections.Generic.List[object]]::new()
  ...
  process {
    ...
    $rows | ForEach-Object {
      ...
      $srcProductPlans = [System.Collections.Generic.List[object]]::new()

      #region Sub-Skus (Plans)
      # Preparing Plans
      if (($srcServicePlan.Trim() -ne '') -and ($srcServicePlanName.Trim() -ne '')) {
        $MessageSuffix = "Reading this requires open and close parenthesis around ServicePlanId - please open issue against Documentation: https://docs.microsoft.com/en-us/azure/active-directory/users-groups-roles/licensing-service-plan-reference"
        ...
        $srcServicePlan -split '<br.?>' | ForEach-Object {
          try {
            ...
            $Message = "Cannot read Entry Service Plan  ('{0}') - malformed string. {1}" -f $planServicePlanName, $MessageSuffix
        ...
        $srcServicePlanName -split '<br.?>' | ForEach-Object {
          try {
            ...
            $Message = "Cannot read Entry Service Plan Name ('{0}') - malformed string. {1}" -f $planProductName, $MessageSuffix
            Write-Warning -Message $Message
            Write-BetterDebug -Message 'srcServicePlan' -OutputObject $srcServicePlan
          }
          ...
          if ($srcProductPlans.ServicePlanId -notcontains $planServicePlanId) {
            try {
              [void]$srcProductPlans.Add([PSstrudelM365LicenseServicePlan]::new($planProductName, $planServicePlanNames[$planServicePlanId], $planServicePlanId, $Relevant))
            }
            catch {
      ...
      $ParameterName = switch ($srcSkuPartNumber) {
        ...
        'DEVELOPERPACK_E5' { 'Microsoft365E5Developer' }
        #endregion
        ...
        # Add-on Licenses
        #TODO check naming and other available Teams Phone licenses
        'MCOEV' { 'PhoneSystem' } # Possible to assign two parameters?
        #'MCOEV' { 'PhoneStandard' } # Additional Service Plans for 'PhonePremium'?
      ...
      # Building Object
      if ( $Products.SkuId -contains $srcSkuId ) {
        $Message = 'License with the same SkuId already present, omitting: {0} | {1} | {2}' -f $ProductName, $srcSkuPartNumber, $srcSkuId
        Write-Warning -Message $Message
      }
      elseif ( $Products.SkuPartNumber -contains $srcSkuPartNumber ) {
        $Message = "License with the same SkuPartNumber present, omitting: {0} | {1} | {2}" -f $ProductName, $srcSkuPartNumber, $srcSkuId
        Write-Warning -Message $Message
      }
      else {
        try {
          [void]$Products.Add([PSstrudelM365LicensePublished]::new( "$ProductName", "$srcSkuPartNumber", "$LicenseType", "$ParameterName", $IncludesTeams, $IncludesPhoneSystem, "$srcSkuId", $srcProductPlans))
        }
        catch {
          $Message = "[PSstrudelM365LicensePublished] Couldn't add entry for '{0}': {1}" -f $ProductName, $_.Exception.Message
    ...
    if ( $ProductsNotAdded.Count -gt 0 ) {
      Write-Warning -Message ('The following Products could not be added: {0}' -f $ProductsNotAdded)
    }
    ...
    if ( -not $ProductsSorted ) {
      Write-Warning -Message ("No Licenses found for SearchString '{0}'" -f $SearchString)
    }
