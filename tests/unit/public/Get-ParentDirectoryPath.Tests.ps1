...
Describe -Tags ('Unit', 'Acceptance') "Function '$(((Split-Path -Leaf $PsCommandPath) -replace '\.Tests\.', '.') -replace '\.ps1', '')'" {
  InModuleScope -ModuleName 'PSstrudel' {
    ...
    Context 'Output' {
      ...
      It 'should find the parent directory' {
        $Parent = Get-ParentDirectoryPath -DirectoryName 'C:\Temp\Parent\Directory\Path' -TargetParentDirectory Parent
        $Parent | Should -Be 'C:\Temp\Parent'
      }
    }
